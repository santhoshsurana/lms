...
$sql="SELECT t.id, t.test_name, p.first_name, p.last_name, t.total_amount, t.paid_amount, t.due_amount, t.date FROM patients p, tests t WHERE t.patient_name=p .id AND t.test_type='1' AND t.date BETWEEN ".$from_date_time." AND ".$to_date_time." ORDER BY t.date DESC LIMIT ".$page_back. ", ".$page_back_limit;
$result=mysqli_query($CON,$sql);
$back_page_count=mysqli_num_rows($result);

$sql="SELECT t.id, t.test_name, p.first_name, p.last_name, t.total_amount, t.paid_amount, t.due_amount, t.date FROM patients p, tests t WHERE t.patient_name=p .id AND t.test_type='1' AND t.date BETWEEN ".$from_date_time." AND ".$to_date_time." ORDER BY t.date DESC LIMIT " .$page_next. ", 1"; 
$result=mysqli_query($CON,$sql);
$next_page_count=mysqli_num_rows($result); 

$sql="SELECT SUM(t.total_amount) FROM patients p, tests t WHERE t.patient_name=p .id AND t.test_type='1' AND t.date BETWEEN ".$from_date_time." AND ".$to_date_time;
$result=mysqli_query($CON,$sql);
...
<?php $sql="SELECT t.id, t.test_name, p.first_name, p.last_name, t.total_amount, t.paid_amount, t.due_amount, t.date FROM patients p, tests t WHERE t.patient_name=p .id AND t.test_type='1' AND t.date BETWEEN ".$from_date_time." AND ".$to_date_time." ORDER BY t.date DESC LIMIT " .$page. ", 15";
$result=mysqli_query($CON,$sql);
